...
threading.Thread(target=run_news, daemon=True).start()
...
async def handle_add_member(ctx, member: discord.Member):
    ...
    Embed.set_author(name="Astrobot", icon_url=ASTROBOT_PROFILE_ASSET)
...
async def handle_create_task(ctx, member: discord.Member):
    if check_overdue(ctx.author.display_name) == False:
...
async def handle_submit(ctx, link):
    ...
    channel_id = 1397207767898394755
